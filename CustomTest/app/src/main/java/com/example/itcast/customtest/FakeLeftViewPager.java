 ...
 * 15058152353@163.com
 */
public class FakeLeftViewPager extends ViewGroup {
    //右侧fragment的自定义控件
    ...
    public FakeLeftViewPager(Context context) {
        super(context);
    }

    public FakeLeftViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
        initDragHelper();
    }
    ...
    private void initDragHelper() {
        viewDragHelper = ViewDragHelper.create(this, new ViewDragHelper
           ...
           //     int finalLeft=left;
                ...
                //(int) Math.pow(left,0.95)
                return (int) Math.pow(left,0.95);
            }
            ...
            public void onViewReleased(View releasedChild, float xvel, float
                    yvel) {
                if (releasedChild.getLeft()>rightViewWidth/16||xvel>100) {
                    //先关闭复位
                    close();
                    //再开启动画
                    open();
    ...
    public FakeLeftViewPager(Context context, AttributeSet attrs, int
            defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        rightView.layout(-vpWidth,0,0,vpHeight);
    }
